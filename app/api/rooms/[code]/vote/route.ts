...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  const [existing] = await db.select().from(roomVotes)
    .where(and(eq(roomVotes.roomId, room.id), eq(roomVotes.userId, session.userId)))
    .limit(1)
  ...
  if (existing) {
    ...
    await db.insert(roomVotes).values({ roomId: room.id, userId: session.userId, compositionIndex })
  }
